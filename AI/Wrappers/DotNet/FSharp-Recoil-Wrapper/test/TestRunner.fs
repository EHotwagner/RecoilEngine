/// Simple test runner for F# RecoilEngine wrapper
module TestRunner
...
let runBasicTest () =
    printfn "=== F# RecoilEngine Wrapper - Standalone Test ==="
    printfn ""
    ...
    printfn ""
    printfn "✅ Standalone F# RecoilEngine wrapper test completed successfully!"
    printfn ""
    printfn "Validated features:"
    printfn "• Pure F# data-oriented architecture"
    printfn "• Units of measure type safety (elmo, hp, metal, energy)"
    printfn "• Structure-of-Arrays world state design"
    printfn "• Spatial query operations"
    printfn "• Command validation and execution pipeline"    printfn "• Independent testing without RecoilEngine dependencies"
    printfn "• Engine-agnostic wrapper suitable for any Spring-based RTS"

[<EntryPoint>]
let main _ =
    runBasicTest()
    
    printfn "\nThis demonstrates that the F# RecoilEngine wrapper can be"
    printfn "developed and tested completely independently of the actual engine!"
